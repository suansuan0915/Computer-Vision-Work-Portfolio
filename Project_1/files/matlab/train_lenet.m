...
%% Loading data
fullset = false;
[xtrain, ytrain, xvalidate, yvalidate, xtest, ytest] = load_mnist(fullset);  % form 784-by-all#n matrix

xtrain = [xtrain, xvalidate];  % 784-by-#n matrix
ytrain = [ytrain, yvalidate];
m_train = size(xtrain, 2);  % #n, include xtrain & xvalidate sets
batch_size = 100;
...
params = init_convnet(layers);  % get h, w, c for each layer
...
for l_idx = 1:length(layers)-1
    param_winc{l_idx}.w = zeros(size(param_winc{l_idx}.w));  % zeros matrix w/ same size as w
    param_winc{l_idx}.b = zeros(size(param_winc{l_idx}.b));
end

%% Training the network
new_order = randperm(m_train);  % random #n as (xtrain & xvalidate sets)
xtrain = xtrain(:, new_order);  % (:, n) -select all rows, n cols 
ytrain = ytrain(:, new_order);

curr_batch = 1;

for iter = 1 : max_iter         % select 100 batches for each iteration
    
    if (curr_batch > m_train)   % reorder images (cols), then reselect 100 batches.
    ...
    x_batch = xtrain(:, curr_batch:(curr_batch+batch_size-1));  % A(:,n) is the nth column of matrix A. first 1:100 images columns of all rows
    y_batch = ytrain(:, curr_batch:(curr_batch+batch_size-1));
    curr_batch = curr_batch + batch_size;  % update curr_batch to 101
